wget -q -O - https://dl.google.com/linux/linux_signing_key.pub | sudo apt-key add -
sudo sh -c 'echo "deb [arch=amd64] http://dl.google.com/linux/chrome/deb/ stable main" >> /etc/apt/sources.list.d/google-chrome.list'
sudo apt update
sudo apt install google-chrome-stable
sudo apt-get update
sudo apt-get install apache2
sudo apt update
sudo apt install phpmyadmin php-mbstring php-zip php-gd php-json php-curl
sudo mysql
mysql -u root -p
sudo apt install apache2
sudo ufw app list
sudo ufw allow in "Apache"
sudo ufw status
wget https://files.phpmyadmin.net/phpMyAdmin/5.1.1/phpMyAdmin-5.1.1-all-languages.tar.xz
sudo tar xf phpMyAdmin-5.1.1-all-languages.tar.xz -C /var/www/
sudo ln -s /var/www/phpMyAdmin-5.1.1-all-languages/ /var/www/phpmyadmin
sudo mkdir /var/www/phpmyadmin/tmp
sudo nano /var/www/phpmyadmin/config.inc.php
mysql -u root -p
sudo nano /var/www/phpmyadmin/config.inc.php
mysql -u root -p
sudo apt install phpmyadmin
sudo mysql
sudo phpenmod mbstring
sudo systemctl restart apache2
sudo mysql
sudo mysql uninstall
ls -l
sudo apt update
sudo apt install apache2
sudo ufw app list
sudo ufw allow 'Apache'
sudo ufw status
sudo apachectl restart
sudo apachectl stop
clear
chmod 755 xampp-linux-*-installer.run
sudo ./xampp-linux-*-installer.run
xampp run
run xampp
sudo run xampp
sudo xampp
clear
ls -l
cd xampp
cd/xampp
xampp-linux-x64-7.4.8-0-installer.run
xampp
cd
xampp
cd
sudo /opt/lampp/lampp start
npx create-react-app todo-app
sudo apt install npm
npx create-react-app todo-app
cd todo-app
npm start
npm i @emotion/styled
npm start
composer create-project --prefer-dist laravel/laravel todo-api
sudo apt install composer
composer create-project --prefer-dist laravel/laravel todo-api
create database laravel_todo;
sudo apt install pbuilder-scripts
sudo apt update
sudo apt install mysql-server
xammp
run xammp
sudo /opt/lampp/lampp start
npm run start
cd
cd todo-api
cd todo-app
cd todo-api
create database laravel_todo;
pcreate database laravel_todo;
php artisan make:model Todo -m
sudo /opt/lampp/manager-linux-x64.run
EDITOR $(which google-chrome)
(which google-chrome)
-user-data-dirno
sudo /opt/lampp/manager-linux-x64.run
sudo -i
npm install bootstrap reactstrap
cat /proc/sys/fs/inotify/max_user_watches
fs.inotify.max_user_watches=524288
sudo sysctl -p
npm install --save react-router-dom
npm install @material-ui/core
npm install @material-ui/icons
npm install @material-ui/core@next
npm install @material-ui/styles
npm i @material-ui/styled-engine
npm i @emotion/react
npm i @material-ui/styled-engine@5.0.0-alpha.20
sudo ./xampp-linux-*-installer.run
create database laravel_todo;
sudo chmod a+rX /root/
sudo chown -R netdata:netdata /root/
sudo /opt/lampp/manager-linux-x64.run
sudo service mysql start
sudo /opt/lampp/manager-linux-x64.run
sudo service mysql start
sudo /opt/lampp/lampp start
php artisan migrate
cd todo-app
cd todo-api
create database laravel_todo;
sudo apt install create
php artisan migrate
cd mysql
php artisan migrate
php artisan serve
sudo ./xampp-linux-*-installer.run
chmod 755 xampp-linux-*-installer.run
sudo ./xampp-linux-*-installer.run
sudo /opt/lampp/lampp start
cd todo-app
cd todo-api
sudo ln -s /opt/lampp/lampp /etc/init.d/lampp
sudo update-rc.d lampp start 80 2 3 4 5 . stop 30 0 1 6
sudo /opt/lampp/lampp start
npm run
npm run start
npm install
npm run start
npm run
cd 
cd todo-app
npm run start
git config --global user.email "you@example.com"
git config --global user.email "asgardtravel@gmail.com"
git config --global user.name "Gormm1980"
sudo/opt/lampp/lampp start
chmod 755 xampp-linux-*-installer.run
sudo ./xampp-linux-*-installer.run
sudo /opt/lampp/lampp start
sudo /opt/lampp/lampp
sudo /opt/lampp/lampp start xammp
tail -2 /opt/lampp/logs/error_log
sudo/opt/lampp/etc/httpd.conf, /opt/lampp/etc/extra/httpd-xampp.conf
apt get update
get apt update
apt update
cd
ls -l
cd snap
ls -l
cd todo-app
cd todo-api
sudo /opt/lampp/lampp start
sudo /opt/lampp/lampp start
cd
cd Descargas
sudo ./xampp-linux-*-installer.run
cd
cd Escritorio
sudo ./xampp-linux-*-installer.run
chmod 755 xampp-linux-*-installer.run
sudo ./xampp-linux-*-installer.run
chmod 755 xampp-linux-*-installer.run
sudo ./xampp-linux-*-installer.run
sudo service mysql start
sudo/opt/lampp/lampp start
cd todo-app
npm update
cd todo-api
sudo apt/lampp/lampp start
sudo ./xampp-linux-*-installer.run
cd
cd escritorio
cd Escritorio
sudo ./xampp-linux-*-installer.run
cd
sudo /opt/lampp/manager-linux-x64.run
sudo service mysql start
sudo /opt/lampp/manager-linux-x64.run
sudo /opt/lampp/lampp start
cd todo-app
npm run start
npm install react-bootstrap@next bootstrap@5.1.0
npm run start
sudo /opt/lampp/lampp start
sudo apt-get install mysql-server mysql-client
sudo service mysql start
sudo apt-get install phpmyadmin php-mbstring php-gettext
service apache2 restart
sudo apt update
sudo apt install phpmyadmin php-mbstring php-zip php-gd php-json php-curl
sudo apt install apache2
sudo service mysql --configure
sudo service mysql status
sudo service mysql restart
apt-get install phpmyadmin
apt install mysql-server
ls -l
sudo ln -s /opt/lampp/lampp /etc/init.d/lampp
sudo update-rc.d lampp start 80 2 3 4 5 . stop 30 0 1 6 
sudo opt/lampp/lampp start
cd
sudo opt/lampp/lampp start
sudo opt/lampp/xampp start
cd equipo
cd 
apt-get install mysql-server mysql-client
sudo apt-get update
sudo apt-get install mysql-server mysql-client
sudo mysql -u root -p
cat /proc/sys/fs/inotify/max_user_watches
fs.inotify.max_user_watches=524288
clear
sudo service apache2 status
sudo service apache2 restart
sudo service httpd restart
sudo opt/lampp/lampp start
sudo /opt/lampp/lampp start
sudo /etc/init.d/apache2 stop
sudo /opt/lampp/lampp start
sudo ./manager-linux-x64.run
sudo /opt/lampp/manager-linux-x64.run
cd todo-app
cd todo-api
php artisan migrate
mysql -u root -p
sudo opt/lampp/lampp start
sudo /opt/lampp/lampp start
php artisan migrate
php artisan migrate:fresh
php artisan passport:install
php artisan passport:install --uuids
php artisan passport:install
php artisan passport:install --uuids
php artisan passport:install
composer require laravel/passport
php artisan migrate
php artisan passport:install
php artisan make:controller UserController
php artisan make:controller TodoController --resource
sudo /opt/lampp/manager-linux64.run
sudo/opt/lampp/manager-linux64.run
sudo /opt/lampp/lmpp start
sudo ln -s /opt/lampp/lampp /etc/init.d/lampp
sudo update-rc.d lampp start 80 2 3 4 5 . stop 30 0 1 6 .
sudo/opt/lampp/manager-linux-x64.run
sudo /opt/lampp/lampp start
sudo /etc/init.d/apache2 stop
sudo /opt/lampp/lmpp start
sudo /opt/lampp/lampp start
cd todo-app
npm run start
cd todo-app
cd todo api
cd todo-api
php artisan run
php artisan schedule:run
php artisan route:list
php artisan migrate:fresh
php artisan migrate
mysql_upgrade 
sudo /opt/lampp/lampp start
sudo /opt/lampp/manager-linux-x64
sudo /opt/lampp/manager-linux-x64.run
cd
sudo -i
sudo/opt/lampp/manager-linux-x64.run
sudo /opt/lampp/manager-linux-x64.run
sudo /etc/init.d/apache2 stop
mysql install
sudo mysql install
cd todo-app
cd todo-api
php artisan migrate
php artisan migrate:fresh
ls /var/lib/mysql
php artisan route:list
sudo ls /var/lib/mysql
clear
php artisan migrate:fresh
php artisan migrate:refresh
cd /var/lib/mysql/laravel_todo
sudo cd /var/lib/mysql/laravel_todo
sudo cd/var/lib/mysql/laravel_todo
cd /var/lib/mysql
cd sudo /var/lib/mysql
sudo /var/lib/mysql
sudo /var/lib/mysql/
clear
sudo /mysql_upgrade
chmod 755 xampp-linux-*-installer.run
sudo ./xampp-linux-*-installer.run
npm install
composer install
php composer install
npm run start
php install
git status
git add
git add .
npm install
git clone
cleaar
clear
git config --global user.name "Gormm1980"
git config --global user.email "asgardtravel@gmail.com"
npm install
sudo apt-get update
git remote add origin https://github.com/Gormm1980/GSR.git
git branch -M main
git push -u origin main
